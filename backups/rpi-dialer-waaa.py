...
##                print str(readflag) + ' Serial read: ' + str(incoming)
                if incoming.startswith('+CBC'):
                    readflag = 0
                    print 'Batt updated'
                    battStatPos = incoming.find(": ")
                    battData = incoming[(battStatPos+2):]
                    battDataList = self.toList(battData)
                    ...
                    signalStatPos = incoming.find(": ")
                    signalData = incoming[(signalStatPos+2):]
                    signalDataList = self.toList(signalData)
