...
def fonaCommand(req):
    ...
    while noreply:
        reply = ser.readline()
        if reply.startswith('OK') or reply.startswith('NO CARRIER') or reply.startswith('ERROR') or reply.startswith('>') or reply.startswith('+'):
            return reply
            noreply = 0
...
class TutorialApp(App):
    ...
    battery_list = ListProperty(['25', '50', '75', '100', 'charge'])
    signal_list = ListProperty(['25', '50', '75', '100', 'no_signal'])
    battery = StringProperty()
    signalstrength = StringProperty()
    batt_level = NumericProperty(0)
    signal_level = NumericProperty(0)
    ...
    def update(self, *args):
        ...
        print categorize_battery(csvToList(checkLevel('AT+CBC')))
        print categorize_signal(csvToList(checkLevel('AT+CSQ')))
        # For testing purposes
        self.battery = self.battery_list[self.batt_level]
        self.signalstrength = self.signal_list[self.signal_level]
        #print(self.battery + " " + self.signalstrength)
        self.batt_level += 1
        self.signal_level += 1
        if self.batt_level >= 5 or self.signal_level >= 5:
            self.batt_level = 0
            self.signal_level = 0
